...
env.hosts = ['46.101.123.6']  # replace with IP address or hostname
env.user = 'michele'
# env.password = 'blah!'
...
def configure_nginx():
    ...
    if exists('/etc/nginx/sites-enabled/uniwherebot') is False:
        sudo('touch /etc/nginx/sites-available/uniwherebot')
        sudo('ln -s /etc/nginx/sites-available/uniwherebot' +
             ' /etc/nginx/sites-enabled/uniwherebot')
    with lcd(local_config_dir):
        with cd(remote_nginx_dir):
            put('./uniwherebot', './', use_sudo=True)
...
def configure_supervisor():
    ...
    """
    if exists('/etc/supervisor/conf.d/uniwherebot.conf') is False:
        with lcd(local_config_dir):
            with cd(remote_supervisor_dir):
                put('./uniwherebot.conf', './', use_sudo=True)
...
def configure_git():
    ...
    if exists(remote_git_dir) is False:
        ...
        with cd(remote_git_dir):
            sudo('mkdir uniwherebot.git')
            with cd('uniwherebot.git'):
                sudo('git init --bare')
                with lcd(local_config_dir):
                    with cd('hooks'):
...
def run_app():
    """ Run the app! """
    with cd(remote_flask_dir):
        sudo('supervisorctl start uniwherebot')
...
def deploy():
    ...
    with lcd(local_app_dir):
        ...
        sudo('supervisorctl restart uniwherebot')
